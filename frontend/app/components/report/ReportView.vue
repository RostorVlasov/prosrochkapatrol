    ...
    <div class="min-h-screen bg-stone-100 dark:bg-stone-900 py-12 px-4">
        <div class="max-w-xl mx-auto">
            ...
            <div class="mb-8">
                ...
                <div
                    class="border-l-2 border-amber-600 dark:border-amber-500 pl-4 py-1 bg-amber-50 dark:bg-amber-950/40 rounded-r-lg">
                    <p class="text-sm leading-relaxed text-stone-700 dark:text-stone-300">
                        Важно: мы проводим мониторинг
                        <strong class="text-amber-700 dark:text-amber-400 font-medium">только в Астрахани</strong>.
            ...
            <form class="space-y-7" @submit.prevent="handleSubmit">
                ...
                <div class="space-y-1.5">
                    <label for="address" class="block text-sm font-medium text-stone-800 dark:text-stone-200">
                        ...
                        <span class="text-amber-600 dark:text-amber-400 ml-0.5">*</span>
                    </label>
                    <input id="address" v-model="form.store_address" type="text" required
                        class="w-full px-3.5 py-2.5 rounded-lg bg-white dark:bg-stone-800 border border-stone-300 dark:border-stone-600 text-sm text-stone-900 dark:text-stone-100 placeholder:text-stone-400 outline-none focus:border-amber-500 focus:ring-3 focus:ring-amber-500/15 transition-all" />
                    ...
                    <div class="space-y-2">
                        <label v-for="option in problemOptions" :key="option.value"
                            ...
                            <input v-model="form.problem_types" type="checkbox" :value="option.value"
                                class="mt-0.5 size-4 rounded border-stone-300 dark:border-stone-600 accent-amber-600 cursor-pointer shrink-0"
                    ...
                    <div class="space-y-2">
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <input v-model="form.staff_contacted" type="radio" value="yes"
                                class="mt-0.5 size-4 border-stone-300 dark:border-stone-600 accent-amber-600 cursor-pointer shrink-0"
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="description" v-model="form.description" required rows="5"
                        class="w-full px-3.5 py-2.5 rounded-lg bg-white dark:bg-stone-800 border border-stone-300 dark:border-stone-600 text-sm text-stone-900 dark:text-stone-100 placeholder:text-stone-400 outline-none focus:border-amber-500 focus:ring-3 focus:ring-amber-500/15 transition-all resize-y" />
                    <p class="text-xs text-stone-400 dark:text-stone-500 leading-relaxed">
                ...
                <div class="pt-1 flex flex-col gap-3">
                    <button type="submit" :disabled="isSubmitting || hasActiveUploads"
                        class="inline-flex items-center gap-2 self-start px-5 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-700 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-medium transition-all cursor-pointer">
