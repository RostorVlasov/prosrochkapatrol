<template>
    <footer class="relative border-t border-beige-100 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden">
        <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(214,180,120,0.06),transparent_35%)]" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        >
                            <span class="font-semibold text-slate dark:text-white">{{ detail.label }}:</span>
                            <span class="ml-1 text-gray-500 dark:text-beige-100/60 group-hover:text-beige-500">{{ detail.value }}</span>
                        </a>
                    </div>
                </div>

                <div class="flex flex-col gap-2 items-start">
                    <div class="text-[10px] uppercase tracking-[0.2em] text-gray-400 dark:text-beige-100/40">Документы</div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <a
                            ...
                            class="group text-xs transition-colors duration-300"
                        >
                            <span class="text-gray-500 dark:text-beige-100/60 group-hover:text-beige-500">{{ doc.label }}</span>
                        </a>

                        <NuxtLink class="text-gray-500 text-xs transition-colors duration-300 dark:text-beige-100/60 hover:text-beige-500" to="/privacy">
                ...
                <div class="flex items-center gap-2 md:justify-end">
                    <a
                        ...
                        class="group flex h-9 w-9 items-center justify-center rounded-lg border border-beige-100 dark:border-gray-700 bg-white/70 dark:bg-gray-800/40 hover:border-beige-300"
                    >
                        <AppIcon :name="link.icon" class="w-4 h-4 text-beige-500 group-hover:scale-110 transition-transform duration-300" />
                    </a>
                </div>
            </div>

            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 pt-5 mt-5 border-t border-beige-100 dark:border-gray-700">
                <div class="w-max">
                    <Mervik />
                </div>

                <p class="max-w-4xl text-[11px] text-gray-400 dark:text-beige-100/40 md:text-right">
                    *Термины «проверка», «проверяющий», «проверили» и подобные используются как синонимы «общественного мониторинга» и не наделяют участников властными или надзорными полномочиями.
                </p>

                <p class="max-w-4xl text-[11px] leading-relaxed text-gray-400 dark:text-beige-100/40 md:text-right">
                    © 2025–{{ currentYear }}
                    <span class="font-bold text-slate dark:text-white">FreshCheck</span>
                    — независимая гражданская инициатива и не подменяет работу официальных государственных контролирующих органов. Основатель проекта: Роман Трошин.
                </p>
            </div>
        </div>
...
const documents = [
  { label: 'Публичная политика', href: PUBLIC_POLICY_DOC_URL },
  { label: 'Бейджи', href: BADGES_DOC_URL },
  { label: 'Декларация', href: DECLARATION_DOC_URL },
  { label: 'Положение', href: REGULATION_DOC_URL }
]
